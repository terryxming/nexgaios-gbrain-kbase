...
Format:
- [Markdown / table / JSON / report / checklist]
...
Required:
- Source text, prompt, SKILL.md content, or workflow instruction to clean.
...
Defaults:
- Preserve operationally useful instructions.
- Remove filler, flattery, repetition, vague quality adjectives, and unnecessary role-play.
- Keep the original task intent unchanged.

Missing Information Handling:
- If the target use case is missing, infer it from the text.
- If the content is too short, clean only obvious noise.

Invalid Inputs:
- Do not rewrite content to bypass safety, compliance, or review processes.
```
...
Format:
- Return the cleaned version first.
- Then return a change summary if useful.

Required Sections:
1. Cleaned Content
2. Removed Noise
3. Remaining Risks or Ambiguities

Quality Requirements:
- Preserve useful constraints.
- Remove redundant instructions.
- Keep the content maintainable.
- Do not weaken safety or factuality rules.
```
...
- It addresses the user’s task directly.
- It separates facts, assumptions, and recommendations.
- It avoids unsupported claims.
- It handles missing information explicitly.
- It is reusable for the intended workflow.
- It avoids filler, repetition, and vague praise.
```
...
- Avoid references to the original conversation.
- Preserve reusable concepts rather than temporary task instructions.
- Include definitions, boundaries, adjacent concepts, use cases, pitfalls, and checklists when supported by source content.
- Avoid unsupported facts.
- Be directly storable in a knowledge base.
```
...
- It invents facts, data, claims, or product features.
- It ignores the required output format.
- It produces a generic answer that could apply to any task.
- It omits key sections required by the workflow.
- It hides uncertainty.
- It includes conversational filler or repeated instructions.
```
