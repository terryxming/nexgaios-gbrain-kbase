...
- Do not store sensitive user data in examples or tests.
- Do not output sensitive raw content unless required for the task.
- Mask or summarize sensitive information when possible.

## Script Safety

- Scripts should be read-only by default.
- Do not delete or overwrite source files unless explicitly requested.
- Do not access network resources unless explicitly approved.
- Do not run unreviewed scripts from external sources.

## External Connections

- Use only approved domains, tools, APIs, or MCP servers.
- Keep a human in the loop for destructive or hard-to-reverse actions.
