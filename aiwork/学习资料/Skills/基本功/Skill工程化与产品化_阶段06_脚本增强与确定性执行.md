...
- State that script-based validation or transformation could not be completed.
- Continue with manual/LLM-based best-effort processing only if safe.
- Do not claim deterministic validation passed.
- Preserve the error message if it helps debugging.
```
...
- Prefer read-only scripts when possible.
- Do not delete or overwrite source files unless explicitly requested.
- Write output to a new file by default.
- Do not access network resources unless the user explicitly requests it and the environment permits it.
