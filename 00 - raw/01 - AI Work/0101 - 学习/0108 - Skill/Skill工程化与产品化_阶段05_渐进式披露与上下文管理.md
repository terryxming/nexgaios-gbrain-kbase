---
title: 'Skill 工程化与产品化｜阶段五：渐进式披露与上下文管理'
status: raw
created: '2026-06-09 21:01'
source_type: unknown
material_type: 普通笔记
domain_hint: 'AI Work'
...
# 第 24 章：Progressive Disclosure（渐进式披露） 原理
...
description: 用于分析 Amazon 产品评论，识别用户痛点、好评驱动因素、购买顾虑、功能需求和产品改进机会。
---
```
...
- `references/brand-voice.md`：在重写面向品牌的文案时读取。
- `references/claim-rules.md`：在检查产品声明时读取。
- `assets/a-plus-module-template.md`：当用户要求输出 Amazon A+ 模块时使用。
- `scripts/validate_markdown.py`：在创建或编辑 Markdown 知识库文件时运行。
```
...
本文件定义 LLM Wiki Markdown 条目的风格规则。

# 使用场景

在创建或修订知识库 Markdown 条目时读取本文件。

# 不适用场景

不要将本文件用于营销文案、 casual summaries（随意摘要）或客户支持消息。
```
...
- 当用户要求输出知识库 Markdown 内容时，读取 `references/llm-wiki-style-guide.md`。
- 当清理对话文本时，读取 `references/forbidden-conversational-phrases.md`。
- 仅当需要通过示例解决歧义时，读取 `references/examples.md`。
```

不要写：

```markdown
开始前，先读取 references 文件夹中的所有文件。
```
...
- 创建新的 LLM Wiki 条目时，使用 `assets/knowledge-entry-template.md`。
- 当输出需要操作型检查清单时，使用 `assets/checklist-template.md`。
- 除非用户要求更新模板，否则不要修改资产模板。
```
...
- `references/style-guide.md`：当任务需要语气、风格或术语对齐时读取。
- `references/examples.md`：当需要通过示例解决输入或输出预期的歧义时读取。
- `references/taxonomy.md`：当需要将内容分类到标准类别时读取。
- `assets/output-template.md`：生成最终交付内容时使用。
- `scripts/validate.py`：验证输出结构时运行。
```
